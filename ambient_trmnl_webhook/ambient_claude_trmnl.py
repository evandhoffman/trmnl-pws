...
async def post_to_webhook(data: dict):
    ...
    if "lastRain" in webhook_data:
        webhook_data["last_rain_date_pretty"] = format_last_rain(
            webhook_data["lastRain"]
        )
    ...
    if "winddir" in webhook_data and "windspeedmph" in webhook_data:
        ...
        webhook_data["winddir_pretty"] = (
            f"{webhook_data['windspeedmph']} mph from the {cardinal}"
        )

    # Format the data to match the expected structure
    payload = {"merge_variables": webhook_data}
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            logger.info(
                f"Successfully posted data to webhook. Status: {response.status_code}"
            )
...
async def main():
    ...
    logger.info(
        f"Starting weather collection service. Interval: {INTERVAL_SECONDS} seconds"
    )
