    ...
    def _parse_csv(self, csv_data: str) -> List[Dict[str, Any]]:
        ...
        for i in range(data_start + 1, len(lines)):
            ...
            if len(values) != len(headers):
                logger.warning(
                    f"Row {i} has {len(values)} values but expected {len(headers)}"
                )
                continue
...
def execute_query(
    ...
    for record in results:
        ...
        if "_value" in record and "_time" in record:
            ...
            clean_record = {"_time": record["_time"], "_value": record["_value"]}
...
def process_solar_data(
    current_records: List[Dict[str, Any]], daily_records: List[Dict[str, Any]], config
) -> Dict[str, Any]:
    """
    ...
    for entity_id, data in sensors_data.items():
        ...
        str_data_str = str(data).replace(
            "'", ""
        )  # Remove single quotes to make it JS-compatible
...
def send_to_webhook(url: str, data: Dict[str, Any]) -> bool:
    ...
    if payload_size > max_payload_size_bytes:
        logger.error(
            f"*** Payload size: {payload_size} bytes, max is {max_payload_size_bytes} bytes ***"
        )
        return False
    else:
    ...
    try:
        ...
        if response.status_code >= 200 and response.status_code < 300:
            ...
            logger.error(
                f"Failed to send data to webhook: {response.status_code} - {response.text}"
            )
            return False
...
def main():
    ...
    client = InfluxDBClient(
        url=influx_url, token=influx_token, org=influx_org, verify_ssl=verify_ssl
    )
    ...
    while True:
        try:
            ...
            logger.info(
                f"Querying current power data for the past {hours_back} hours ({power_start_time} to {power_end_time})"
            )

            # Calculate time range for daily energy data
            energy_start_time, energy_end_time = calculate_daily_range(days_back)
            logger.info(
                f"Querying daily energy data for the past {days_back} days ({energy_start_time} to {energy_end_time})"
            )
            ...
            if not current_records and not daily_records:
                ...
                # Process data
                processed_data = process_solar_data(
                    current_records, daily_records, config
                )
            ...
            logger.info(
                f"Waiting {poll_interval} seconds before next data collection..."
            )
